...
useRef는 input요소에 사용 또는 단독으로 이전값 사용을 위해 사용. */
...
//input과 연결 없이 사용하는 예시 (따라서, value 속성 안씀)
  const [count, setCount] = useState(0)
  const prevCount = useRef(0) // 이 변수는 객체이므로 current 속성으로 값 설정

  useEffect(() => {
    prevCount.current = count
  }, [count]) // count가 바뀔 때 마다 콜백함수 실행, 빈 배열로 바꾸면 마운트하고 실행해서 0으로 유지된다.
